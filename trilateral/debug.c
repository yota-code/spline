...
typedef struct {
 ...
 patch_C;
pos_T * kernel_pointer = ((void *)0);
patch_C kernel__new__(size_t radius) {
 patch_C new;
 new.radius = radius;
...
int kernel__del__(patch_C * self) {
 free(self->kernel);
 free(self);
 return 0;
}
...
real_T kernel_quantile(patch_C * self, real_T q) {
 return quantile(self->kernel, self->length, q);
}
int kernel_square_sort(patch_C * self) {
 qsort(self->kernel, self->length, sizeof (real_T), _compare);
 return 0;
}
int kernel_square__load__(patch_C * self, char * filename) {
 FILE * fid;
 size_t len;
 char * buffer, * cell;
 ...
 i++) {
  if (buffer[i] == '\t' || buffer[i] == '\n') {
   ...
   ((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) = value;
   if (buffer[i] == '\t') {
    x += 1;
   }
   if (buffer[i] == '\n') {
...
int kernel_square__dump__(patch_C * self, char * filename) {
 FILE * fid;
 size_t len;
...
int kernel_square__display__(patch_C * self) {
 for (size_t i=0 ;
 i<self->width ;
 i++) {
  for (size_t j=0;
...
int kernel_pointer_scanline(patch_C * self) {
 for (int i=0 ;
 i<self->length ;
 i++) {
...
int pointer_set_spiral(patch_C * self) {
 int x = 0, y = 0;
 int dx = 0, dy = -1, b = 0;
 for (int i=0 ;
 i<self->length ;
...
int kernel_square_distance(patch_C * self, double norm) {
 {
 for (int i=0 ;
 i < self->length ;
 i++) {
 int x = self->pointer[i].x;
 int y = self->pointer[i].y;
 ((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) = _distance2(x, y, norm);
 }
 }
;
 return 0;
}
int mask_set_circle(patch_C * self, double radius, double norm) {
 real_T d;
 {
 for (int i=0 ;
 i < self->length ;
 ...
 ((self)->mask[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) = (d <= radius) ? 1.0 : 0.0;
 }
 }
}
int kernel_square_normalize_center(patch_C * self) {
 real_T n = ((self)->kernel[(0 + ((self)->radius)) + ((0 + ((self)->radius)) * ((self)->width))]);
 ((n != 0.0) ? (void) (0) : __assert_fail ("n != 0.0", "kernel3.c", 249, __PRETTY_FUNCTION__));
 {
 for (int i=0 ;
 i < self->length ;
 i++) {
 int x = self->pointer[i].x;
 int y = self->pointer[i].y;
 ((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) = ((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) / n;
 }
 }
;
}
int kernel_square_normalize(patch_C * self, double n) {
 ((n != 0.0) ? (void) (0) : __assert_fail ("n != 0.0", "kernel3.c", 256, __PRETTY_FUNCTION__));
 {
 for (int i=0 ;
 i < self->length ;
 i++) {
...
double kernel_square_max(patch_C * self) {
 real_T z;
 {
 for (int i=0 ;
 i < self->length ;
 ...
 int x = self->pointer[i].x;
 int y = self->pointer[i].y;
 z = ((z) > (((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))])) ? (z) : (((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))])));
 }
 }
;
}
double kernel_square_sum(patch_C * self) {
 real_T z;
 {
 for (int i=0 ;
 i < self->length ;
 ...
 int x = self->pointer[i].x;
 int y = self->pointer[i].y;
 z += ((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]);
 }
 }
;
}
int kernel_square_normalize_max(patch_C * self) {
 real_T n = kernel_square_max(self);
 ((n != 0.0) ? (void) (0) : __assert_fail ("n != 0.0", "kernel3.c", 280, __PRETTY_FUNCTION__));
 {
 for (int i=0 ;
 i < self->length ;
 i++) {
...
int kernel_square_where_lower_equal(patch_C * self, double value) {
 {
 for (int i=0 ;
 i < self->length ;
 i++) {
 int x = self->pointer[i].x;
 int y = self->pointer[i].y;
 ((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) = ((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) <= value ? 1.0 : 0.0;
 }
 }
;
 return 0;
}
plane_T kernel_square_average_gradient(patch_C * self) {
 real_T a = 0.0, b = 0.0, c = 0.0, g;
 real_T d = ((self)->kernel[(0 + ((self)->radius)) + ((0 + ((self)->radius)) * ((self)->width))]);
 {
 for (int i=0 ;
 i < self->length ;
 i++) {
 int x = self->pointer[i].x;
 int y = self->pointer[i].y;
 g = ((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) - d;
 a += -1.0 * g * x;
 b += -1.0 * g * y;
...
plane_T patch_biased_gradient(patch_C * self, plane_T p) {
...
;
 real_T d, m, n;
 q.d = ((self)->kernel[(0 + ((self)->radius)) + ((0 + ((self)->radius)) * ((self)->width))]);
 {
 for (int i=0 ;
 i < self->length ;
 i++) {
 int x = self->pointer[i].x;
 int y = self->pointer[i].y;
 d = ((p.a*x) + (p.b*y) + (p.c*((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))])) + p.d) / e;
 m = ((self)->mask[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) * (1.0/(1.0 + pow(d, 2.0)));
 n = ((self)->kernel[(x + ((self)->radius)) + ((y + ((self)->radius)) * ((self)->width))]) - q.d;
 q.a += -1.0 * n * x * m;
 q.b += -1.0 * n * y * m;
